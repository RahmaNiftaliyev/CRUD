...
function exportDataController() {
    ...
    if ([productName, productSeller, productPrice,].some(i => i === "")) {
        ...
        ajax.post(
            new Product(productName, productQuantity, productSeller, productPrice, ajax.setId(1, INPUT_ID)),
            (err, response) => err ? alertify.error(`error is ${err}`) :
...
function DOMController() {
    ajax.get(
        (err, response, responseLength) => {
            if (err) {
                alertify.error(`error is ${err}`);
            } else {
                alertify.success(`last updated product: ${response[responseLength - 1].name}`)
...
function importDataController() {
    ...
    if (stock > 0 && store === 0) {
        ...
        stock === 0 ?
            ...
            counter === 0 ?
                alertify.warning('No any new Item') : dataImporter(stock, counter, store) //access new products
    }
}
...
function inputValueController(event) {
   ...
   let classBorder = Array.from(event.target.classList);
    ...
    if(classBorder.indexOf('border-danger') !== -1 && event.target.value !== "") {
...
function UIDataChangerController(event) {
    ...
    if(event.target.className === 'fas fa-edit') {
            ...
            BUTTON_UPDATE.hasAttribute('disabled') ?
                BUTTON_UPDATE.removeAttribute('disabled'):null
    }
    else if(event.target.className === 'far fa-trash-alt'){
            deleteIcon = !editIcon;

            !BUTTON_UPDATE.hasAttribute('disabled') ?
                BUTTON_UPDATE.setAttribute('disabled','disabled'):null
    }
